...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  $insertGoTo = "partidas_proyecto.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
?>
			...
			<input name="COD_PROYECTO" type="hidden" id="COD_PROYECTO" value="<?php echo $_POST['CODIGO']; ?>" /></td>
		</tr>
		<tr>
...
do {  
?>
			<option value="<?php echo $row_partidas['ID']?>"<?php if (!(strcmp($row_partidas['ID'], 0))) {echo "selected=\"selected\"";} ?>>[ID:<?php echo $row_partidas['ID']?>] - [Disponible:<?php echo number_format($row_partidas['MONTO_ESTIMADO_DISPONIBLE'],2)?>] - <?php echo $row_partidas['DESCRIPCION_COMPLETA']?> </option>
			<?php
...
?>
		...
		<tr>
			...
			<td><label for="total_impuesto"></label>
				<input name="total_impuesto" type="text" class="textos_form_derecha" id="total_impuesto" value="0" readonly="readonly" /></td>
		</tr>
		<tr>
			<td class="textos_form">Total:</td>
			<td><label for="total"></label>
				<input name="total" type="text" class="textos_form_derecha required" id="total" value="0" readonly="readonly" /></td>
		</tr>
				<tr>
			<td colspan="2" class="textos_form" align="center"><?php if (validador1(12,$_SESSION['i'],"oth")==1){ ?><input name="button" type="submit" class="ui-button" id="button" value="Guardar" /><?php } ?></td>
		</tr>
	</table>
