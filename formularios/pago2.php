...
$query_documentos = sprintf("SELECT * FROM vista_documentos WHERE MODULO='1' AND STATUS_DOCUMENTO=1 AND MONTO_DOCUMENTO>0 AND COD_PROYECTO='".$_POST['CODIGO']."' AND ID_PRO_CLI = %s ORDER BY FECHA_DOCUMENTO", GetSQLValueString($colname_documentos, "int"));
...
mysql_select_db($database_conexion, $conexion);
$query_tipo_pago = "SELECT * FROM tipo_pago ORDER BY TIPO ASC";
...
<table width="1100" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<select name="TIPO_PAGO" id="TIPO_PAGO">
			...
			do {  
			?>
			<option value="<?php echo $row_tipo_pago['TIPO']?>"<?php if (!(strcmp($row_tipo_pago['TIPO'], 2))) {echo "selected=\"selected\"";} ?>><?php echo $row_tipo_pago['DESCRIPCION']?></option>
			<?php
