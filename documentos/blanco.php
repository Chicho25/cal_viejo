...
<form action="../consulta_pagos/pago3.php" method="post">
...
<tr><td colspan="9" align="center"><table border="0" cellpadding="8" cellspacing="8">
	<tr>
		<td><?php if ($pageNum_reporte > 0) { // Show if not first page ?>
				<a href="<?php printf("%s?pageNum_reporte=%d%s", $currentPage, 0, $queryString_reporte); ?>"><img src="../consulta_pagos/First.gif" /></a>
				<?php } // Show if not first page ?></td>
		<td><?php if ($pageNum_reporte > 0) { // Show if not first page ?>
				<a href="<?php printf("%s?pageNum_reporte=%d%s", $currentPage, max(0, $pageNum_reporte - 1), $queryString_reporte); ?>"><img src="../consulta_pagos/Previous.gif" /></a>
				<?php } // Show if not first page ?></td>
		<td><?php if ($pageNum_reporte < $totalPages_reporte) { // Show if not last page ?>
				<a href="<?php printf("%s?pageNum_reporte=%d%s", $currentPage, min($totalPages_reporte, $pageNum_reporte + 1), $queryString_reporte); ?>"><img src="../consulta_pagos/Next.gif" /></a>
				<?php } // Show if not last page ?></td>
		<td><?php if ($pageNum_reporte < $totalPages_reporte) { // Show if not last page ?>
				<a href="<?php printf("%s?pageNum_reporte=%d%s", $currentPage, $totalPages_reporte, $queryString_reporte); ?>"><img src="../consulta_pagos/Last.gif" /></a>
				<?php } // Show if not last page ?></td>
	</tr>
</table>
</tr></table>
