...
mysql_select_db($database_conexion, $conexion);
$query_partidas2 = "SELECT * FROM partidas";
$partidas2 = mysql_query($query_partidas2, $conexion) or die(mysql_error());
$row_partidas2 = mysql_fetch_assoc($partidas2);
$totalRows_partidas2 = mysql_num_rows($partidas2);
...
<form action="listado2.php" method="get" id="enviar">
...
?>
    ...
    <tr>
          ...
          <td bgcolor="#F3F3F3" ><label for="ESTIMADO_EXCEDIDO"></label>
            ...
            <input name="col" type="hidden" id="col" value="ALICUOTA, ORDEN" />          <input name="orden" type="hidden" id="orden" value="1" />          <input name="MONTO_ESTIMADO" type="hidden" id="MONTO_ESTIMADO" />          <input name="DESCRIPCION_COMPLETA" type="hidden" id="DESCRIPCION_COMPLETA" />          <input name="MONTO_ASIGNADO" type="hidden" id="MONTO_ASIGNADO" />          <input name="MONTO_PAGADO" type="hidden" id="MONTO_PAGADO" />          </tr>
    <tr>
          <td align="right" bgcolor="#F3F3F3" class="textos_form_derecha" >Partida:
      <td bgcolor="#F3F3F3" ><span id="spryselect1">
      <select name="ID_PARTIDA" id="combobox" title="cliente" style="width: 300px; font-size:10px" width="300">
      	<option value="" <?php if (!(strcmp("", 0))) {echo "selected=\"selected\"";} ?>></option>
      	<?php
do {  
?>
      	<option value=" AND ID_GRUPO=<?php echo $row_proveeedor['ID']?>"<?php if (!(strcmp($row_proveeedor['DESCRIPCION_COMPLETA'], 0))) {echo "selected=\"selected\"";} ?>>[ID:<?php echo $row_proveeedor['ID']?>] <?php echo $row_proveeedor['DESCRIPCION_COMPLETA']?></option>
      	<?php
...
?>
      ...
      <span class="selectRequiredMsg">Deben seleccionarse dos partidas.</span></span><!--     	<input name="DETALLE" type="checkbox" id="DETALLE" value="1" />
	  ...
	  <tr>
          <td align="right" bgcolor="#F3F3F3" class="textos_form_derecha" >Partida:
      <td bgcolor="#F3F3F3" ><span id="spryselect2">
      <select name="ID_PARTIDA2" id="combobox2" title="cliente" style="width: 300px; font-size:10px" width="300">
      	<option value="" <?php if (!(strcmp("", 0))) {echo "selected=\"selected\"";} ?>></option>
      	<?php
do {  
?>
      	<option value=" AND ID_GRUPO=<?php echo $row_partidas2['ID']?>"<?php if (!(strcmp($row_partidas2['DESCRIPCION_COMPLETA'], 0))) {echo "selected=\"selected\"";} ?>>[ID:<?php echo $row_partidas2['ID']?>] <?php echo $row_partidas2['DESCRIPCION_COMPLETA']?></option>
      	<?php
} while ($row_partidas2 = mysql_fetch_assoc($partidas2));
  $rows = mysql_num_rows($partidas2);
  if($rows > 0) {
      mysql_data_seek($partidas2, 0);
	  $row_partidas2 = mysql_fetch_assoc($partidas2);
  }
?>
      	</select>
      <span class="selectRequiredMsg">Deben seleccionarse dos partidas</span></span>
      	<label for="DETALLE"></label></tr>
