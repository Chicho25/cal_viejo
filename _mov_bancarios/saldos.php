...
PROYECTOS.NOMBRE AS NOMBRE_PROYECTO,
 `banco_master`.`NOMBRE` AS NOMBRE_BANCO
  ...
  FORMAT(SUM(IF((`tesoreria_tipo_mov`.`TIPO_IO` = _latin1'I'),`mov_banco_caja`.`MONTO`,0))-SUM(  IF((`tesoreria_tipo_mov`.`TIPO_IO` = _latin1'O'),`mov_banco_caja`.`MONTO`,0)),2) AS saldo
...
FROM
    ...
    INNER JOIN `grupocal_calpe`.`banco_master` 
        ...
        WHERE `tesoreria_tipo_mov`.AFECTA_BANCO=1
