...
$query_CONSULTA = "DROP VIEW IF EXISTS `PRUEBA_NUEBA`";
$QUERY = mysql_query($query_CONSULTA, $conexion) or die(mysql_error());

$query_CONSULTA = "CREATE VIEW `grupocal_calpe`.`PRUEBA_NUEBA` 
    AS
(SELECT 
  inmuebles_mov.ID_INMUEBLES_MOV,
  vista_inmuebles.CODIGO_INMUEBLE,
  vista_inmuebles.NOMBRE_PROYECTO AS PROYECTO,
  vista_inmuebles.NOMBRE_GRUPO,
  vista_inmuebles.ID_GRUPO,
  vista_inmuebles.COD_PROYECTO AS ID_PROYECTO,
  vista_inmuebles.ID_INMUEBLE,
...
vista_ventas_comisiones.NOMBRE_VENDEDOR
  

FROM
  inmuebles_mov
  LEFT OUTER JOIN vista_inmuebles ON (vista_inmuebles.ID_INMUEBLE = inmuebles_mov.ID_INMUEBLES_MASTER)
  LEFT OUTER JOIN pro_cli_master ON (inmuebles_mov.ID_PRO_CLI_MASTER = pro_cli_master.ID_PRO_CLI_MASTER)

LEFT OUTER JOIN vista_ventas_comisiones ON (vista_ventas_comisiones.ID_INMUEBLES_MOV = inmuebles_mov.ID_INMUEBLES_MOV AND vista_ventas_comisiones.PORCENTAJE_COMISION = 0.26000)

WHERE
  inmuebles_mov.ID_INMUEBLES_MOV > 0
GROUP BY
  inmuebles_mov.ID_INMUEBLES_MOV
ORDER BY
  inmuebles_mov.ID_INMUEBLES_MOV)"
...
$query_CONSULTA ="DROP VIEW IF EXISTS `PRUEBA_NUEVA2`";
$QUERY = mysql_query($query_CONSULTA, $conexion) or die(mysql_error());
$query_CONSULTA ="CREATE
 
    VIEW `PRUEBA_NUEVA2` 
    AS
(SELECT 
  prueba_nueba.ID_INMUEBLES_MOV,
  prueba_nueba.CODIGO_INMUEBLE,
  prueba_nueba.PROYECTO,
  prueba_nueba.NOMBRE_GRUPO,
  prueba_nueba.ID_GRUPO,
  prueba_nueba.ID_PROYECTO,
  prueba_nueba.ID_INMUEBLE,
  prueba_nueba.FECHA_VENTA,
  prueba_nueba.FECHA_VENTA_DATE,
  prueba_nueba.ID_CLIENTE,
  prueba_nueba.NOMBRE_CLIENTE,
  prueba_nueba.MONTO_VENTA,
  
prueba_nueba.NOMBRE_VENDEDOR,

  documentos.ID_INMUEBLES_MOV AS TIENE_DOCUMENTOS,
  SUM((documentos.MONTO_EXENTO + documentos.MONTO_GRABADO) + documentos.MONTO_IMPUESTO) AS MONTO_DOCUMENTO
FROM
  documentos
  RIGHT OUTER JOIN prueba_nueba ON (documentos.ID_INMUEBLES_MOV = prueba_nueba.ID_INMUEBLES_MOV)
GROUP BY
  prueba_nueba.ID_INMUEBLES_MOV)";
$QUERY = mysql_query($query_CONSULTA, $conexion) or die(mysql_error());
...
$query_CONSULTA = "SELECT 
  prueba_nueva2.ID_INMUEBLES_MOV,
  prueba_nueva2.CODIGO_INMUEBLE,
  prueba_nueva2.PROYECTO,
  prueba_nueva2.NOMBRE_GRUPO,
  prueba_nueva2.ID_GRUPO,
  prueba_nueva2.ID_PROYECTO,
  prueba_nueva2.ID_INMUEBLE,
  prueba_nueva2.FECHA_VENTA,
  prueba_nueva2.FECHA_VENTA_DATE,
  prueba_nueva2.ID_CLIENTE,
  prueba_nueva2.NOMBRE_CLIENTE,
  prueba_nueva2.MONTO_VENTA,
  
prueba_nueva2.NOMBRE_VENDEDOR,

  prueba_nueva2.TIENE_DOCUMENTOS,
  prueba_nueva2.MONTO_DOCUMENTO,
  IF((prueba_nueva2.MONTO_VENTA = prueba_nueva2.MONTO_DOCUMENTO),1,0) AS `VENTA_CERRADA`
FROM
  prueba_nueva2
  ".$where." ORDER BY ".$ordenar;
//echo $query_CONSULTA;
...
<?php if ($totalRows_CONSULTA > 0) { // Show if recordset not empty ?>
  <table width="1100" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#999999">
    <tr class="menu">
      ...
      <td width="100" align="center" bgcolor="#cccccc" class="textos_form">VENDEDOR</td>
...
$TOTAL=$TOTAL+$row_CONSULTA['MONTO_VENTA'];
	?>
      <tr style="font-family:Arial, Helvetica, sans-serif; font-size:12px" bgcolor='<?php echo $color ?>' >
        ...
        <td><?php echo $row_CONSULTA['NOMBRE_VENDEDOR']; ?></td>
