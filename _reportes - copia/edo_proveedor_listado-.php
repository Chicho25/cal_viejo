...
<?php 
$NUMERO_COLUMNAS=9;
$titulo="Estado de Cuenta Proveedor";
$titulo_col1="ID";
$titulo_col2="Concepto";
$titulo_col3="Tipo";
$titulo_col4="Numero";
$titulo_col5="ID Partida";
$titulo_col6="Fecha Venc.";
...
$query_CONSULTA = "SELECT 'a',ID_DOCUMENTO, CONCEPTO, TIPO, NUMERO, ID_PARTIDA, FECHA_VENCIMIENTO, DESCRIPCION, FORMAT(DEBITO,2), FORMAT(CREDITO,2),DEBITO,CREDITO,  NOMBRE_PRO_CLI  FROM vista_edo_cuenta_proveedores WHERE MODULO=1 AND ID_PRO_CLI='".$_GET['PROVEEDOR']."' AND CODIGO_PROYECTO='".$_GET['CODIGO_PROYECTO']."' ".$_GET['COMISIONES']." ".$id_partida." ; ";
...
<?php if ($totalRows_CONSULTA > 0) { // Show if recordset not empty ?>
	<table width="1100" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#999999">
		...
		<!--Fin Encabezado de las columnas-->
				...
				$saldo=0;
				$color='e0e0e0';
				$id_documento=$row_CONSULTA[1];
				do { 
				//echo "1:".$row_CONSULTA[10];
					if($id_documento!=$row_CONSULTA[1]){
						$id_documento=$row_CONSULTA[1];
						//echo 1;
						if($color=='e0e0e0'){
							$color='ffffff';
