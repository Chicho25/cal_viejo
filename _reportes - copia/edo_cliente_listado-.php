...
<?php 
$NUMERO_COLUMNAS=8;
$titulo="Estado de Cuenta Cliente";
$titulo_col1="Concepto";
$titulo_col2="Tipo";
$titulo_col3="Numero";
$titulo_col4="Apt";
$titulo_col5="Fecha Venc.";
$titulo_col6="Descripcion";
$titulo_col7="Debito";
$titulo_col8="Credito";
$alineacion_1="center";
$alineacion_2="center";
...
$query_CONSULTA = "SELECT 'a',CONCEPTO, TIPO, NUMERO, CODIGO_INMUEBLE, FECHA_VENCIMIENTO, DESCRIPCION, FORMAT(DEBITO,2), FORMAT(CREDITO,2),DEBITO,CREDITO, ID_DOCUMENTO, NOMBRE_PRO_CLI FROM vista_edo_cuenta_clientes WHERE CODIGO_PROYECTO='".$_GET['CODIGO_PROYECTO']."' ".$_GET['PROVEEDOR']."  ".$_GET['INMUEBLE']."  ; ";
//echo $query_CONSULTA;
...
mysql_select_db($database_conexion, $conexion);
$query_CONSULTA_TOTAL = "SELECT '','', '', '', '','', 'Total', FORMAT(SUM(DEBITO),2), FORMAT(SUM(CREDITO),2) FROM vista_edo_cuenta_clientes WHERE  CODIGO_PROYECTO='".$_GET['CODIGO_PROYECTO']."' ".$_GET['PROVEEDOR']."  ".$_GET['INMUEBLE']."  ; ";
//echo $query_CONSULTA;
...
<?php if ($totalRows_CONSULTA > 0) { // Show if recordset not empty ?>
	<table width="1100" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#999999">
		...
		<tr class="menu">
			...
			<?php }?>
			<td width="50" align="center" bgcolor="#cccccc" class="textos_form">Saldo<?php //echo $saldo; ?></td>
		</tr>
		<!--Fin Encabezado de las columnas-->
				<?php 
				$saldo=0;
				$color='e0e0e0';
				$id_documento=$row_CONSULTA[11];
				do { 
				//echo "1:".$row_CONSULTA[10];
					if($id_documento!=$row_CONSULTA[11]){
						$id_documento=$row_CONSULTA[11];
						//echo 1;
						if($color=='e0e0e0'){
							$color='ffffff';
 ...
 ?>
			...
			<?php }?>
			<?php 
			$saldo=$saldo+$row_CONSULTA[9]-$row_CONSULTA[10];			
			?>
