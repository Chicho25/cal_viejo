...
$query_CONSULTA = "SELECT NOMBRE_TIPO_MOV, NUMERO_PAGO, NOMBRE_PROYECTO, NOMBRE_BANCO, NUMERO_CUENTA,ID_MOV_BANCO_CAJA, DESCRIPCION, MES, ANO, DIA, DEBITO, CREDITO FROM vista_banco_movimientos  WHERE AFECTA_BANCO=1 AND ANULADO=0 AND ID_CUENTA_BANCARIA=".$_GET['ID_CUENTA_BANCARIA']." AND MES=".$_GET['MES']." AND ANO=".$_GET['ANO']." ";
...
<form name="enviar" method="POST" id="enviar">
	<?php if ($totalRows_CONSULTA > 0) { // Show if recordset not empty ?>
		<table width="1100" align="center" cellpadding="2" cellspacing="2" >
			...
			<tr>
				<td colspan="8" align="right" bgcolor="#999999" class="textos_form_derecha" >Saldo Anterior			
		   ...
		   ?>
			<tr bgcolor="<?php if($row_CONSULTA['CREDITO']>0){?>#ffffff<?php }else{?>#F3F3F3<?php }?>">
				<td align="center"  class="textos_form" ><?php echo $row_CONSULTA['DIA']; ?>
				...
				<td colspan="2" align="left"  ><?php if($row_CONSULTA['CREDITO']>0){echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";} ?>
					<?php echo $row_CONSULTA['DESCRIPCION']; ?>
			...
			<tr>
				<td colspan="6" align="right" bgcolor="#999999" class="textos_form_derecha" >Saldo Final<td align="right" bgcolor="#999999" class="textos_form_derecha" >	<?php echo number_format($DEBITO,2); ?>		
