...
<?php 
$NUMERO_COLUMNAS=8;
$titulo="Ventas Inmuebles";
$titulo_col3="Apt. Vendidos";
$titulo_col2="Total Apt.";
$titulo_col1="Nombre";
$titulo_col4="Monto Vendido";
$titulo_col5="Monto Por Vender";
$titulo_col6="Monto Cobrado";
$titulo_col7="Monto Por Cobrar";
$titulo_col8="Monto Vencido";
...
$query_CONSULTA = "SELECT ID_GRUPO,  CONCAT('Total ',NOMBRE_GRUPO), COUNT(*) AS APARTAMENTOS,COUNT(IF(MONTO_VENDIDO>0,MONTO_VENDIDO,NULL)) , FORMAT(SUM(MONTO_VENDIDO),2), FORMAT(SUM(MONTO_POR_VENDER),2), FORMAT(SUM(MONTO_COBRADO),2), FORMAT(SUM(MONTO_POR_COBRAR),2), FORMAT(SUM(MONTO_VENCIDO),2) FROM vista_rpt_ventas_inmuebles WHERE CODIGO_PROYECTO=".$_GET['CODIGO_PROYECTO']." GROUP BY ID_GRUPO";//.$where." ORDER BY ".$ordenar;
...
mysql_select_db($database_conexion, $conexion);
$query_CONSULTA_TOTAL = "SELECT ID_GRUPO , 'Total', COUNT(*),COUNT(IF(MONTO_VENDIDO>0,MONTO_VENDIDO,NULL)) ,FORMAT(SUM(MONTO_VENDIDO),2), FORMAT(SUM(MONTO_POR_VENDER),2), FORMAT(SUM(MONTO_COBRADO),2), FORMAT(SUM(MONTO_POR_COBRAR),2), FORMAT(SUM(MONTO_VENCIDO),2),' ',' ' FROM vista_rpt_ventas_inmuebles WHERE CODIGO_PROYECTO=".$_GET['CODIGO_PROYECTO']." ";//.$where." ORDER BY ".$ordenar;
...
<?php if ($totalRows_CONSULTA > 0) { // Show if recordset not empty ?>
	<table width="1100" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#999999">
		...
		<?php do { ?>
		<?php 
		if(isset($_GET['DETALLADO'])){$DETALLADO=$_GET['DETALLADO'];}
		if($DETALLADO==1){?>
		<?php		
	mysql_select_db($database_conexion, $conexion);
	$query_SUB_CONSULTA = "SELECT ID_GRUPO,NOMBRE_INMUEBLE, ' ',' ', FORMAT(MONTO_VENDIDO,2), FORMAT(MONTO_POR_VENDER,2),FORMAT(MONTO_COBRADO,2) , FORMAT(MONTO_POR_COBRAR,2), FORMAT(MONTO_VENCIDO,2), ERROR, VENTA_CERRADA FROM vista_rpt_ventas_inmuebles WHERE ID_GRUPO=".$row_CONSULTA[0];
	$SUB_CONSULTA = mysql_query($query_SUB_CONSULTA, $conexion) or die(mysql_error());
	//$row_SUB_CONSULTA = mysql_fetch_row($SUB_CONSULTA);
	$totalRows_SUB_CONSULTA = mysql_num_rows($SUB_CONSULTA);
	?>
		<?php do { ?>
		<tr style="font-family:Arial, Helvetica, sans-serif; font-size:12px" bgcolor='<?php if (($row_SUB_CONSULTA[9]==0)){ ?>#FFFFFF<?php }else{?>#ffcaca<?php }?>' >
			<?php
		$ALINEACION="left";
		for ($i = 1; $i <= $NUMERO_COLUMNAS; $i++) {
	?>
			<td align="<?php echo $ALINEACION ?>"><?php echo $row_SUB_CONSULTA[$i]; ?></td>
			<?php 
		$ALINEACION="right";
				}?>
		</tr>
		<?php } while ($row_SUB_CONSULTA = mysql_fetch_row($SUB_CONSULTA)); ?>
		<?php } ?>
		
		<tr bgcolor='#cccccc' class="textos_form" >
			<?php $ALINEACION="center";
		for ($i = 1; $i <= $NUMERO_COLUMNAS; $i++) {
		?>
			<td align="<?php echo $ALINEACION ?>"><?php echo $row_CONSULTA[$i]; ?></td>
			<?php 
		$ALINEACION="right";
		}?>
		</tr>
		...
		for ($i = 1; $i <= $NUMERO_COLUMNAS; $i++) {
		?>
			<td align="<?php echo $ALINEACION ?>"><?php echo $row_CONSULTA_TOTAL[$i]; ?></td>
			<?php 
		$ALINEACION="right";
		}?>
		</tr>
