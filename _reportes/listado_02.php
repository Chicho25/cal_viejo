...
<?php 
$NUMERO_COLUMNAS=7;
$titulo="Ventas Inmuebles con Cliente";
//$titulo_col3="Apt. Vendidos";
$titulo_col2="Nombre";
$titulo_col1="Codigo";
$titulo_col3="Monto Vendido";
$titulo_col4="Monto Por Vender";
$titulo_col5="Monto Cobrado";
$titulo_col6="Monto Por Cobrar";
$titulo_col7="Monto Vencido";
$pagina="listado_02.php";
...
$query_CONSULTA = "SELECT ID_GRUPO,  CONCAT('Total ',NOMBRE_GRUPO), '', FORMAT(SUM(MONTO_VENDIDO),2), FORMAT(SUM(MONTO_POR_VENDER),2), FORMAT(SUM(MONTO_COBRADO),2), FORMAT(SUM(MONTO_POR_COBRAR),2), FORMAT(SUM(MONTO_VENCIDO),2) FROM vista_rpt_ventas_inmuebles WHERE CODIGO_PROYECTO=".$_GET['CODIGO_PROYECTO']." GROUP BY ID_GRUPO";//.$where." ORDER BY ".$ordenar;
...
mysql_select_db($database_conexion, $conexion);
$query_CONSULTA_TOTAL = "SELECT ID_GRUPO , 'Total', '',FORMAT(SUM(MONTO_VENDIDO),2), FORMAT(SUM(MONTO_POR_VENDER),2), FORMAT(SUM(MONTO_COBRADO),2), FORMAT(SUM(MONTO_POR_COBRAR),2), FORMAT(SUM(MONTO_VENCIDO),2),' ' FROM vista_rpt_ventas_inmuebles WHERE CODIGO_PROYECTO=".$_GET['CODIGO_PROYECTO']." ";//.$where." ORDER BY ".$ordenar;
...
<?php if ($totalRows_CONSULTA > 0) { // Show if recordset not empty ?>
	<table width="1100" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#999999">
		...
		<?php		
	mysql_select_db($database_conexion, $conexion);
	$query_SUB_CONSULTA = "SELECT ID_GRUPO,CODIGO_INMUEBLE, NOMBRE_CLIENTE, FORMAT(MONTO_VENDIDO,2), FORMAT(MONTO_POR_VENDER,2),FORMAT(MONTO_COBRADO,2) , FORMAT(MONTO_POR_COBRAR,2), FORMAT(MONTO_VENCIDO,2), ERROR, VENTA_CERRADA FROM vista_rpt_ventas_inmuebles WHERE ID_GRUPO=".$row_CONSULTA[0];
	...
	?>
		<?php do { ?>
		<tr style="font-family:Arial, Helvetica, sans-serif; font-size:12px" bgcolor='<?php if (($row_SUB_CONSULTA[8]==0)){ ?>#FFFFFF<?php }else{?>#ffcaca<?php }?>' >
			<?php
		$ALINEACION="center";
		for ($i = 1; $i <= $NUMERO_COLUMNAS; $i++) {
			
			if($i==2){$ALINEACION="left";}else{$ALINEACION="right";};
			if($i==1){$ALINEACION="center";};
	?>
