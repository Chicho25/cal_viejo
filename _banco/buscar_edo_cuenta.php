...
$query_CUENTAS = "select distinct
  `empresas_master`.`CODIGO_EMPRESAS_MASTER` AS `CODIGO_EMPRESA`,
  `empresas_master`.`NOMBRE`                 AS `NOMBRE_EMPRESA`,
  `proyectos`.`CODIGO`                       AS `CODIGO_PROYECTO`,
  `proyectos`.`NOMBRE`                       AS `NOMBRE_PROYECTO`,
  `banco_cuentas`.`ID_BANCO_MASTER`          AS `ID_BANCO_MASTER`,
  `banco_master`.`NOMBRE`                    AS `NOMBRE_BANCO`,
  `banco_master`.`NACIONAL`                  AS `BANCO_NACIONAL`,
  `banco_cuentas`.`ID_CUENTA_BANCARIA`       AS `ID_CUENTA`,
  `banco_cuentas`.`PREDETERMINADA`           AS `PREDETERMINADA`,
  `banco_cuentas`.`NUMERO_CUENTA`            AS `NUMERO_CUENTA`,
  `banco_cuentas`.`DESCRIPCION_CUENTA`       AS `DESCRIPCION_CUENTA`,
  if(isnull(`mov_banco_caja`.`ID_CUENTA_BANCARIA`),0,1) AS `TIENE_MOVIMIENTOS`,
  if(isnull(`banco_chequeras`.`ID_CUENTA_BANCARIA`),0,1) AS `TIENE_CHEQUERAS`
from (((((`empresas_master`
       left join `proyectos`
         on ((`empresas_master`.`CODIGO_EMPRESAS_MASTER` = `proyectos`.`COD_EMP`)))
      join `banco_cuentas`
        on ((convert(`banco_cuentas`.`CODIGO_EMPRESA` using utf8) = `empresas_master`.`CODIGO_EMPRESAS_MASTER`)))
     join `banco_master`
       on ((`banco_cuentas`.`ID_BANCO_MASTER` = `banco_master`.`ID_BANCO_MASTER`)))
    left join `mov_banco_caja`
      on ((`banco_cuentas`.`ID_CUENTA_BANCARIA` = `mov_banco_caja`.`ID_CUENTA_BANCARIA`)))
   left join `banco_chequeras`
     on ((`banco_cuentas`.`ID_CUENTA_BANCARIA` = `banco_chequeras`.`ID_CUENTA_BANCARIA`)))" ;//"SELECT * FROM vista_banco_cuentas";
...
mysql_select_db($database_conexion, $conexion);
$query_ANOS = "SELECT DISTINCT `mov_banco_caja`.`FECHA` AS `FECHA_DATE`, YEAR(`mov_banco_caja`.`FECHA`) AS `ANO` FROM `mov_banco_caja` 
WHERE mov_banco_caja.FECHA IS NOT NULL AND YEAR(mov_banco_caja.FECHA)<>0 GROUP BY YEAR(mov_banco_caja.FECHA)"; //"SELECT ANO FROM vista_banco_movimientos WHERE FECHA_DATE IS NOT NULL AND YEAR(FECHA_DATE)<>0 GROUP BY ANO";
