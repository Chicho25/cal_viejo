...
if ((isset($_GET["MM_insert"])) && ($_GET["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO contabilidad_cuentas (DESCRIPCION, ID_SUCURSAL, ID_GRUPO, TIPO, NIVEL, ID_ALICUOTA, PORCENTAJE_ALICUOTA,  MONTO_ESTIMADO) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
...
mysql_select_db($database_conexion, $conexion);
$query_rst_centro_costos = "SELECT ID_CUENTA FROM vista_contabilidad_cuentas WHERE TIENE_HIJOS =1";
...
?>
    ...
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">
      <?php if ($totalRows_rst_centro_costos > 0){echo "Grupo";} else {echo "Centro de Costos";}?>:</td>
      <td><span id="spryselect1">
        <label for="ID_GRUPO"></label>
    ...
    <div class="alic">
      <tr valign="baseline">
        ...
        <td><input type="text" name="PORCENTAJE_ALICUOTA" id="PORCENTAJE_ALICUOTA" value="0" size="5" /></td>
      </tr>
      <tr valign="baseline">
...
do {  
?>
            <option value="<?php echo $row_rst_partidas['ID_CUENTA']?>"><?php echo $row_rst_partidas['DESCRIPCION']?></option>
            <?php
...
?>
    ...
    <tr valign="baseline">
      ...
      <td><input type="text" name="MONTO_ESTIMADO" value="0.00" id="MONTO" /></td>
    </tr>
    <tr valign="baseline">
      <td colspan="6" align="center" nowrap="nowrap"><input name="titulo_formulario" type="hidden" id="titulo_formulario" value="<?php echo $_GET['titulo_formulario'] ?>" /><input type="submit" value="Aceptar" class="ui-state-hover"/></td>
    </tr>
  </table>
