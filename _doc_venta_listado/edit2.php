...
if($_GET['STATUS']!="Todos"){
		...
		if($_GET['STATUS']==4){
			$where=$where." AND VENCIDO=1 ";
		};
		if($_GET['STATUS']==5){
			$where=$where." AND STATUS_APROBADO=1 ";
		};
		if($_GET['STATUS']==6){
			$where=$where." AND STATUS_APROBADO=0 ";
		};
	}
	/////// nuevos
if(isset($_GET['grupo']) && $_GET['grupo']!='0'){$grupo=' AND ID_GRUPO='.$_GET['grupo'];} else
{$grupo='';}
if(isset($_GET['FROM']) && $_GET['FROM']!='' && isset($_GET['TO']) && $_GET['TO']!='')
{$fi=DMAtoAMD($_GET['FROM']); $ff=DMAtoAMD($_GET['TO']);
	$fecha=' AND FECHA_DOCUMENTO_DATE BETWEEN "'.$fi.'" AND "'.$ff.'"';} else {$fecha='';}
	
if(isset($_GET['FROM1']) && $_GET['FROM1']!='' && isset($_GET['TO1']) && $_GET['TO1']!='')
{$fi1=DMAtoAMD($_GET['FROM1']); $ff1=DMAtoAMD($_GET['TO1']);
	$fecha1=' AND FECHA_VENCIMIENTO_DATE BETWEEN "'.$fi1.'" AND "'.$ff1.'"';} else {$fecha1='';}
...
$query_CONSULTA = "SELECT * FROM vista_documentos_ventas ".$where.$fecha.$fecha1.$grupo." ORDER BY ".$ordenar." ";
...
$(document).ready(function(){
        ...
        function DoSomething() {
            local_id = current_id;
		var url = "../_doc_venta/del.php?ID_DOCUMENTO="+local_id; 
		window.location =   url;   
			...
			<tr>
				<td colspan="2" align="center"><table width="450" border="0" cellpadding="2" cellspacing="0" class="textos_form">
					<tr>
						...
						<td width="100" class="textos_form"><a title="Ver Solo Sin Abonos" href="#" class="textos_ordenar_rojo">Sin Abonos</a></td>
						<td width="50" bgcolor="#FFFF99">&nbsp;</td>
						<td width="100"><a title="Ver Solo Con Abonos" href="#" class="textos_ordenar_rojo">Con Abonos</a></td>
						<td width="50" bgcolor="#B3FFB3">&nbsp;</td>
						<td width="100"><a title="Ver Solo los Pagados" href="#" class="textos_ordenar_rojo">Pagados</a></td>
					</tr>
				</table></td>
				</tr>
		</table>
...
<?php if ($totalRows_CONSULTA > 0) { // Show if recordset not empty ?>
	<table width="1100" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#999999">
		<tr class="menu">
			...
			<td width="150" align="center" bgcolor="#cccccc" class="textos_form">
				<a title="Ordenar" href="edit2.php?elegido=<?php $_GET['elegido']?>&col=CODIGO_INMUEBLE&TIPO=<?php echo $tipo?>&PROYECTO=<?php echo $proyecto; ?>&STATUS=<?php echo $status; ?>&orden=<?php echo $orden;  ?>" class="<?php if ($_GET['col']=="CODIGO_INMUEBLE"){ ?>textos_ordenar_rojo<?php }else{?>textos_ordenar<?php }?>">INMUEBLE  <?php if ($_GET['col']=="CODIGO_INMUEBLE"){ if($_GET['orden']==2){ ?><img src="../image/za.png" width="12" height="13" border="0" /><?php }else{?><img src="../image/az.png" width="12" height="13" border="0" /><?php }}else{?><img src="../image/azbw.png" width="12" height="13" border="0" /><?php }?></a>
			</td>
			...
			<td width="100" align="center" bgcolor="#cccccc" class="textos_form">
				<a title="Ordenar" href="edit2.php?elegido=<?php $_GET['elegido']?>&col=NUMERO_DOCUMENTO&TIPO=<?php echo $tipo?>&PROYECTO=<?php echo $proyecto; ?>&STATUS=<?php echo $status; ?>&orden=<?php echo $orden;  ?>" class="<?php if ($_GET['col']=="NUMERO_DOCUMENTO"){ ?>textos_ordenar_rojo<?php }else{?>textos_ordenar<?php }?>">Numero <?php if ($_GET['col']=="NUMERO_DOCUMENTO"){ if($_GET['orden']==2){ ?><img src="../image/za.png" width="12" height="13" border="0" /><?php }else{?><img src="../image/az.png" width="12" height="13" border="0" /><?php }}else{?><img src="../image/azbw.png" width="12" height="13" border="0" /><?php }?></a>
			</td>
			
			<td width="100" align="center" bgcolor="#cccccc" class="<?php if ($_GET['col']=="FECHA_VENCIMIENTO_DATE"){ ?>textos_ordenar_rojo<?php }else{?>textos_ordenar<?php }?>"><a title="Ordenar" href="edit2.php?elegido=<?php $_GET['elegido']?>&col=FECHA_VENCIMIENTO_DATE&TIPO=<?php echo $tipo?>&PROYECTO=<?php echo $proyecto; ?>&STATUS=<?php echo $status; ?>&orden=<?php echo $orden;  ?>" class="<?php if ($_GET['col']=="FECHA_VENCIMIENTO_DATE"){ ?>textos_ordenar_rojo<?php }else{?>textos_ordenar<?php }?>">Fecha Vencimiento<?php if ($_GET['col']=="FECHA_VENCIMIENTO_DATE"){ if($_GET['orden']==2){ ?><img src="../image/za.png" width="12" height="13" border="0" /><?php }else{?><img src="../image/az.png" width="12" height="13" border="0" /><?php }}else{?><img src="../image/azbw.png" width="12" height="13" border="0" /><?php }?></a></td>
			<td  align="center" bgcolor="#cccccc" class="textos_form">
			<a title="Ordenar" href="edit2.php?elegido=<?php $_GET['elegido']?>&col=elegido&TIPO=<?php echo $tipo?>&PROYECTO=<?php echo $proyecto; ?>&STATUS=<?php echo $status; ?>&orden=<?php echo $orden;  ?>" class="<?php if ($_GET['col']=="NOMBRE_PRO_CLI"){ ?>textos_ordenar_rojo<?php }else{?>textos_ordenar<?php }?>">Cliente  <?php if ($_GET['col']=="NOMBRE_PRO_CLI"){ if($_GET['orden']==2){ ?><img src="../image/za.png" width="12" height="13" border="0" /><?php }else{?><img src="../image/az.png" width="12" height="13" border="0" /><?php }}else{?><img src="../image/azbw.png" width="12" height="13" border="0" /><?php }?></a></td>
			<td align="center" bgcolor="#cccccc" class="textos_form">
	...
	?> >
				...
				<td align="right"><?php echo number_format($row_CONSULTA['MONTO_DOCUMENTO']-$row_CONSULTA['MONTO_PAGADO'],2); ?></td>
				<td align="right"><a href="../_doc_venta/edit.php?ID_DOCUMENTO=<?php echo $row_CONSULTA['ID_DOCUMENTO']; ?>" title="Ver/Editar"><img src="../image/icon_doc.png" width="24" height="24" /></a></td>
				<td align="right" ><?php if ($row_CONSULTA['STATUS_DOCUMENTO']==1){?>
				<!--<a href="../_ventas_pago/add.php?ID_DOCUMENTO=<?php echo $row_CONSULTA['ID_DOCUMENTO']; ?>&ID_INMUEBLES_MOV=<?php echo $row_CONSULTA['ID_INMUEBLES_MOV'] ?>" title="Nuevo Pagos"><img src="../image/dollar.png" width="24" height="28" /></a>-->	
                <a href="../pagos/list.php?titulo_formulario=Recepci%F3n%20de%20Pagos&ID_PAGO=&modulo=2" title="Nuevo Pagos"><img src="../image/dollar.png" width="24" height="28" /></a>				<?php }?></td>
				<td align="right" ><?php if ($row_CONSULTA['TIENE_PAGOS']==0){?>
				<a href="#" title="Eliminar Registro" ><img alt="Eliminar Registro"  src="../image/Delete-icon.png" width="24" height="24" onClick="openDialog(<?php echo $row_CONSULTA['ID_DOCUMENTO']; ?>)" /></a><?php }?></td>
      ...
      <td  align="right" bgcolor="#cccccc" class="textos_form"><?php echo number_format($TOTAL1-$TOTAL2,2); ?></td>
